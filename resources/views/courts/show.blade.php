        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6 mb-6 overflow-x-auto">
                ...
                <div class="flex items-center justify-between mb-4">
                    <a href="{{ route('courts.show', [$court, 'week' => $startOfWeek->copy()->subWeek()->format('Y-m-d')]) }}"
                    ...
                    <a href="{{ route('courts.show', [$court, 'week' => $startOfWeek->copy()->addWeek()->format('Y-m-d')]) }}"
                       class="px-3 py-1 bg-gray-100 rounded hover:bg-gray-200">次の週 ＞</a>
                </div>

                @php
                    $dowJa = ['月', '火', '水', '木', '金', '土', '日'];
                    $hours = range(9, 20); // 9:00〜21:00
                    $days = collect();
                    for ($i = 0; $i < 7; $i++) {
